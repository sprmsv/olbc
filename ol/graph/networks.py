...
from ol.graph.entities import TypedGraph, EdgesIndices, EdgeSetKey, EdgeSet, NodeSet, Context
...
def _node_update(graph: TypedGraph, node_fn: GNUpdateNodeFn,
  ...
  sum_n_node = jax.tree.leaves(node_set.features)[0].shape[1]
  ...
  for edge_set_key, edge_set in graph.edges.items():
    ...
    if sender_node_set_key == node_set_key:
      ...
      senders = edge_set.indices.senders
      sent_features[edge_set_key.name] = jax.tree.map(
  ...
  for edge_set_key, edge_set in graph.edges.items():
    ...
    if receiver_node_set_key == node_set_key:
      assert isinstance(edge_set.indices, EdgesIndices)
      receivers = edge_set.indices.receivers
      received_features[edge_set_key.name] = jax.tree.map(
...
def _global_update(graph: TypedGraph, global_fn: GNUpdateGlobalFn,
  ...
  for edge_set_key, edge_set in graph.edges.items():
    ...
    edge_features[edge_set_key.name] = jax.tree.map(
  ...
  for node_set_key, node_set in graph.nodes.items():
    sum_n_node = jax.tree.leaves(node_set.features)[0].shape[0]
    n_node = node_set.n_node[0]
    node_gr_idx = jnp.repeat(graph_idx, n_node, axis=1, total_repeat_length=sum_n_node)
    node_features[node_set_key] = jax.tree.map(
...
def InteractionNetwork(
  ...
  wrapped_update_edge_fn = jax.tree.map(lambda fn: lambda e, s, r, g, **kw: fn(e, s, r, **kw), update_edge_fn)
  ...
  if include_sent_messages_in_node_update:
    wrapped_update_node_fn = jax.tree.map(lambda fn: lambda n, s, r, g, **kw: fn(n, s, r, **kw), update_node_fn)
  else:
    wrapped_update_node_fn = jax.tree.map(lambda fn: lambda n, s, r, g, **kw: fn(n, r, **kw), update_node_fn)
