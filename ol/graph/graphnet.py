...
from ol.graph.entities import TypedGraph
from ol.graph.networks import GraphMapFeatures, InteractionNetwork
from ol.models.common import FeedForward
...
class DeepTypedGraphNet(nn.Module):
  ...
  * num_message_passing_steps = N, num_processor_repetitions = M, gives
  ...
  def _embed(self, input_graph: TypedGraph, **kwargs) -> TypedGraph:
    ...
    if jax.tree.leaves(context_features):
      # This code assumes a single input feature array for the context and for
      # each node type.
      assert len(jax.tree.leaves(context_features)) == 1
      new_nodes = {}
